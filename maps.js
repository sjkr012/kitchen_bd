...
document.addEventListener("DOMContentLoaded", function() {
	...
	//////////////////// L6M 히트맵 ////////////////////
	var map7 = new naver.maps.Map(document.getElementById('map7'), {
	zoom: 5,
	mapTypeId: 'normal',
	center: new naver.maps.LatLng(35.868, 128.599)
	});	
...
////////////////////////////////////////// map 7 /////////////////////////////////////
			...
			var circle7 = new naver.maps.Circle({
			    map: map7,
			    center: new naver.maps.LatLng(x, y),
			    radius: dist,
				strokeColor: 'red',
			...
			var orders = 0;
			var cnt = 0;
			naver.maps.Event.once(map7, 'init_stylemap', function () {
			      $.getJSON("./busan_l6m_order_re+call.json", function(data){
			    	  //console.log(data['coordinates'][0]['location'][0]);
			    	  
			    	  for(let i=0; i < data['coordinates'].length; i++) {
			    		  lng = data['coordinates'][i]['location'][1];
			    		  lat = data['coordinates'][i]['location'][0];
			    		  
			    		  //dt = getDist(y,x,lat,lng);
			    		  //dt = getDist(lat,lng,y,x); changed version
			    		  dt = getDist(lng,lat,x,y);
			    		  if (dt <= dist) { 
			    			  cnt++;
			    			  orders = orders + data['coordinates'][i]['weight'];
			    			  orders = orders + data['coordinates'][i]['call'];
			    		  } else {
			    		  	 //data['coordinates'][i]['location'][1] = 0;
			    		  	 //data['coordinates'][i]['location'][0] = 0;
			    		  	 //data['coordinates'][i]['weight'] = 0;
		    			  }
			    	  }
			    	  
			    	  document.querySelector('#total7').innerHTML = orders;
			    	  document.querySelector('#total8').innerHTML = cnt;
			...
			// Facility location
			var circle7_x = new naver.maps.Circle({
			    map: map7,
			    center: new naver.maps.LatLng(x, y),
			    radius: 10,
				strokeColor: 'black',
